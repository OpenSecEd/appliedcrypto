...
\begin{frame}
  \begin{definition}[\Acl{DLP}, \acs{DLP}]
    \begin{itemize}
      \item Let \(\Z_n^*\) be the multiplicative group of residues modulo 
        \(n\in \N\).

        \pause{}

        \begin{description}
          \item[Given] \(g, g^x\in \Z_n^*\)
          \item[Find] \(x\).
        \end{description}

      \item I.e.\ compute \(\log_{g\in \Z_n}(g^x)\).
    \end{itemize}
  \end{definition}
\end{frame}
...
\begin{frame}
  ...
  \begin{definition}[\Acl{DDH} Problem, \acs{DDH}]
    \begin{description}
      \item[Given] \(g, g^x, g^y, g^z\in \Z_n^*\)
...
\begin{frame}
  \begin{definition}[Diffie-Hellman key-exchange]
    \begin{itemize}
      ...
      \color{green!50!red}
      \item Alice generates random \(0 < x < |\Z_n^*|\).
      \item She send \(g^x\) to Bob.

        \pause{}

      \color{blue}
      \item Bob generates random \(0 < y < |\Z_n^*|\).
...
\begin{frame}
  \begin{definition}[ElGamal Encryption Scheme\footfullcite{ElGamal}]
    Set-up:
    \begin{itemize}
      \item Let \(g\in \Z_n^*\), randomly choose \(0 < x < |\Z_n^*|\).
      \item Alice publishes \(\Z_n^*, g, g^x\) to everyone.
    \end{itemize}
    Encryption:
    \begin{itemize}
      \item Bob chooses random \(0 < y < |\Z_n^*|\) and computes \(g^y\).
      \item Bob's message \(m\in \Z_n^*\).
...
\begin{frame}
  \begin{definition}[ElGamal Signature Scheme\footfullcite{ElGamal}]
    Set-up:
    \begin{itemize}
      \item Let \(g\in \Z_n^*\) and \framebox{\(h\) be a one-way function}.
      \item Alice publishes \(\Z_n^*, g, g^x\) to everyone.
    \end{itemize}
    Signing \(m\in \Z_n^*\):
    \begin{itemize}
      \item \framebox{Alice} chooses random \(0 < y < |\Z_n^*|\) and computes 
        \(r = g^y\in \Z_n^*\).
      \item She computes \framebox{\(s = (h(m) - xr)\pmod{|\Z_n^*|}\).}
      \item She sends \((r, s)\) to Bob.
    \end{itemize}
    Verification:
    \begin{itemize}
      \item \framebox{Bob checks if \(g^{h(m)} \stackrel{?}{=}_{\Z_n^*} (g^x)^r 
            r^s\)}\( =_{\Z_n^*}
          (g^x)^{g^y} (g^y)^{h(m) - xg^y} =_{\Z_n^*}
          g^{xg^y + h(m) - xg^y}\)
    \end{itemize}
  \end{definition}
\end{frame}

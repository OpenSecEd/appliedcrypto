...
% XXX add description of Vigénère and point out that it was believed to be 
...
% XXX move frame about breaking the Hill cipher here.
...
% XXX add question about alternatives to known plaintext attacks.
...
\Ac{CBC} mode XORs each plaintext block with the previous ciphertext block 
before encryption.
This breaks the pattern leakage of \ac{ECB}: even identical plaintext blocks 
produce different ciphertext blocks (unless they appear after identical 
prefixes).
\Ac{CBC} requires an \ac{IV} for the first block, which must be random and 
unpredictable.
...
\Ac{CTR} mode turns a block cipher into a stream cipher.

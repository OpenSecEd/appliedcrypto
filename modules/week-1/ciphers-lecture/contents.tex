...
\begin{description}
  \item[Shared-key (symmetric) cryptography]
    The oldest form, where the same secret key is shared between the 
  ...
  \item[Public-key (asymmetric) cryptography]
    A revolutionary development from the 1970s.
  ...
  \item[Counter-intuitive cryptography]
    The most modern developments, from the 1980s onward.
    These include zero-knowledge proofs (proving you know something without 
    revealing what), secure multi-party computation (computing on secret 
    inputs without revealing them), and other seemingly impossible feats.
\end{description}
...
\begin{frame}
  \begin{definition}[Perfect secrecy]\footfullcite{ShannonSecrecy}
    ...
    The system has \emph{perfect secrecy} if and only if 
...
The \emph{one-time pad} (OTP) achieves perfect secrecy.
...
\begin{frame}
  \begin{example}[One-time Pad]
    Let \(n\) be a positive integer and let \(\M = \C = \K = (\ZZ_2)^n\).

    \pause{}

    For each key \(k = (k_1, \ldots, k_n)\in \K\), plaintexts 
    ...
    We also define \(\Dec = \Enc\).

    \pause{}

    The key \(k\in \K\) must be chosen uniformly at random for each encryption.
  \end{example}
\end{frame}
...
Key distribution and the prohibition on reuse make the OTP impractical for 
most applications.

In practice, we settle for \emph{computational} security rather than 
information-theoretic security.
We use ciphers based on \emph{pseudorandom permutations} (PRPs): functions 
that are indistinguishable from truly random permutations by any efficient 
algorithm.
...
A PRP is a function that, when keyed, behaves like a random permutation to 
anyone who does not know the key.
Modern block ciphers like AES are designed to be PRPs: given a 128-bit key, 
AES maps 128-bit blocks to 128-bit blocks in a way that is computationally 
...
\begin{frame}
  \begin{definition}[Substitution cipher]
    The key is a permutation \(\sigma\) of the alphabet.
    Encryption replaces each letter: \(c_i = \sigma(m_i)\).
    Decryption inverts: \(m_i = \sigma^{-1}(c_i)\).
  \end{definition}
\end{frame}
...
\begin{frame}
  \begin{definition}[Vigen\`ere cipher]
    The key is a short sequence \(k_0,\ldots,k_{\ell-1}\) that is reused 
    periodically.
    Encryption shifts each letter by the corresponding key element:
    \(c_i = m_i + k_{i\bmod\ell} \bmod n\).
    Decryption reverses: \(m_i = c_i - k_{i\bmod\ell} \bmod n\).
  \end{definition}
\end{frame}

The Vigenère cipher fell to cryptanalysis in the 19th century.
...
\begin{frame}
  \begin{definition}[Hill cipher]
    The key is an invertible \(m \times m\) matrix \(K\) over \(\ZZ_n\).
    The plaintext is grouped into vectors \(\mathbf{p} = (p_1, \ldots, p_m)^T\) 
    of \(m\) letters.
    Encryption multiplies: \(\mathbf{c} = K \cdot \mathbf{p} \bmod n\).
    Decryption inverts: \(\mathbf{p} = K^{-1} \cdot \mathbf{c} \bmod n\).
  \end{definition}
\end{frame}
...
standard phrases.
The Hill cipher succumbs immediately: given \(m\) known plaintext-ciphertext 
block pairs, the attacker can set up a system of linear equations and solve 
for the key matrix.
...
\begin{frame}
  \begin{block}{Substitution-permutation networks (SPNs)}
    \begin{itemize}
  ...
  \ltnote{%
    ...
    \textbf{What remains invariant}: The SPN structure.
...
The \emph{Advanced Encryption Standard} (AES) is the most widely used block 
cipher today.
It uses a 128-bit block size and supports key sizes of 128, 192, or 256 bits.
AES is an SPN: each round applies a nonlinear substitution (SubBytes), a 
permutation (ShiftRows), a mixing operation (MixColumns), and a key addition 
(AddRoundKey).
After 10--14 rounds (depending on key size), the result is a ciphertext that 
appears random to anyone without the key.

\begin{frame}
  \begin{block}{AES at a glance}
    \begin{itemize}
...
\begin{frame}
  \begin{block}{Quantum threat: Grover's algorithm}
    \begin{itemize}
      ...
      \item AES-256 is quantum-safe at the 128-bit level.
    \end{itemize}
  \end{block}
  ...
  \ltnote{%
    ...
    \textbf{What remains invariant}: The cipher itself (AES-256).
...
256-bit keys.
AES-256 is therefore considered quantum-safe at the 128-bit security level.
...
A block cipher encrypts fixed-size blocks (for example, 128 bits for AES).
...
This naive approach is called \emph{Electronic Codebook} (ECB) mode.
...
The famous \enquote{ECB penguin} demonstrates this dramatically.
When a bitmap image of the Linux mascot Tux is encrypted with ECB mode, the 
penguin is still clearly visible in the ciphertext.
With a proper mode like CTR, the ciphertext appears random.
...
\emph{Cipher Block Chaining} (CBC) mode XORs each plaintext block with the 
previous ciphertext block before encryption.
This breaks the pattern leakage of ECB: even identical plaintext blocks 
produce different ciphertext blocks (unless they appear after identical 
prefixes).
CBC requires an \emph{initialization vector} (IV) for the first block, which 
...
\emph{Counter} (CTR) mode turns a block cipher into a stream cipher.
The cipher encrypts a sequence of counter values (nonce concatenated with a 
counter) to produce a keystream, which is XORed with the plaintext.
This is highly parallelizable and allows random access to any block.
However, nonce reuse is catastrophic: if the same nonce is used twice, the 
...
encryption.
This is provided by an \emph{initialization vector} (IV) or \emph{nonce}.
The exact requirements vary by mode: CBC needs an unpredictable IV, while CTR 
only requires the nonce to be unique (never reused with the same key).
...
The consequences of nonce reuse in CTR mode are immediate and devastating.
...
The lesson is clear: IV/nonce discipline is not optional---it is part of the 
security definition.
...
Modes like GCM (Galois/Counter Mode) are widely used for this purpose.
For specialized applications like disk encryption, modes like XTS are 

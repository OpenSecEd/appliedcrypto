  ...
  \begin{msc}
    ...
    {Command from \(H\) to \(D_i\)}
      ...
      \mscaction{If \(t\) invalid, abort; otherwise: \begin{align*}
          m_{\text{ack}} &= \text{\enquote{ack: }} \concat m \\
      ...
      \nextlevel
      \mscaction{\begin{align*}
          m_{\text{report}} &= \text{\enquote{report: result of exec}} \\
...
\begin{enumerate}
  ...
  \item It computes the MAC: \(t = \MAC[\KMHD, c]\)---remember that the 
    \(\MAC\) will use the hash value of \(c\), not \(c\) itself.
...
would think that the changer will suspend charging whereas it will not.
We need to acknowledge and confirm the command to ensure that the controller 
knows whether the command was received and executed successfully.
If the controller doesn't receive an acknowledgment, it can retry or alert the 
user.
\begin{enumerate}[resume]
  \item The device constructs an acknowledgment message \(m_{\text{ack}} =
    \text{\enquote{ack: suspend until 14:00}}\).
...
\begin{enumerate}
  ...
  \item Unspecified what \(\Enc\) does for large messages, ECB?
...
\ltnote{%
  We now keep the protocol structure invariant and vary only the concrete
  realization of \(\Enc\).
  ...
  This also prepares a later generalization about randomness sources without
  yet changing the trust or pairing assumptions.
}

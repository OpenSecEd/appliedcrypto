...
\]
and compute an HMAC~\autocite{HMAC} to authenticate it:
\[
...
To prevent this, we include a nonce in the token; the box stores the nonces
of used tokens and rejects any token whose nonce it has already seen.
...
BLE provides several pairing methods with different security
levels~\autocite{BluetoothCore}:
\begin{description}
  ...
  \item[Out-of-Band (OOB)] Uses a separate channel (\ac{NFC}, QR code) to
...
installation.
The key is stored in a secure element (\ac{TPM} or similar) within the box.
...
information.
Under the \ac{GDPR} and similar regulations, this personal data must be
protected.
...
\begin{enumerate}
  ...
  \item Photo storage creates \ac{GDPR} compliance obligations.
...
This is similar to JSON Web Signatures (JWS)~\autocite{RFC7515} used in
many web authentication systems.

\begin{exercise}
  What algorithm should we use for the signatures?
\end{exercise}

We should use ECDSA (Elliptic Curve Digital Signature
Algorithm)~\autocite{FIPS186-5} with a curve like P-256 or P-384.
ECDSA provides strong security with much smaller signatures and keys than
RSA, which is important for resource-constrained boxes communicating via
BLE\@.
(RSA verification is actually faster than ECDSA's, thanks to the small
public exponent, but over BLE the signature size matters more.)
...
\begin{description}
  \item[Android: Play Integrity API] The app requests an integrity verdict from 
    ...
    Store, and whether the device passes integrity checks (not rooted,
    bootloader locked, etc.).
...
One approach uses blind signatures~\autocite{ecash}.
...
The customer proves (perhaps using zero-knowledge
techniques~\autocite{GMR1989zk}) that they have a valid delivery
notification for \emph{some} box, without revealing which one.

This is complex to implement correctly.
Simpler alternatives include:
\begin{itemize}
  \item Using an anonymizing proxy or onion
    routing~\autocite{OnionRouting1996,Tor} so the
    server sees the request but not who sent it.
...
\begin{enumerate}
  ...
  \item Blind tokens alone do not hide everything: the server assigned the
    parcel to a compartment at delivery, so a compartment-open event
    reported by the box still identifies the parcel---and thus the
    customer---unless the box's access logs are minimized as well.
...
\begin{description}
  \item[\ac{GDPR}] Personal data (access logs, photos) must be protected.
...
\begin{description}
  \item[Rate limiting] Lock out after a few failed attempts on the
    keypad.
  \item[Time limits] Fallback codes expire quickly (\eg 30 minutes).
  \item[Audit logging] All fallback access attempts are logged and
    flagged for review.
  \item[Notification] The primary phone (if reachable) receives a
    notification when fallback access is used.
  \item[Value-based restrictions] High-value parcels may not allow
    fallback access; the customer must use primary authentication or visit
    customer service.
\end{description}
...
\begin{figure}
  ...
  \begin{msc}
    ...
    {Multi-provider access}
      ...
      \nextlevel[2]
      \mscaction{Issue customer token}{prov}
      \nextlevel[2]
      \mess{Token + authorization}{prov}{cust}
      \nextlevel[2]
      \mess{Token + authorization (via BLE)}{cust}{box}
      \nextlevel
...
\begin{enumerate}
  ...
  \item \ac{PKI} with certificate chains for trust establishment.
  \item Secure hardware (\ac{TPM}, secure boot) for implementation
    security.
  \item \ac{GDPR}-compliant data handling with appropriate retention
    policies.
  \item Fallback authentication (rate-limited one-time codes) for
    availability when the app-based method fails.
  \item A federated multi-provider architecture mediated by the box
    operator.
\end{enumerate}
...
\ltnote{%
  ...
  time in this chapter---token design and freshness, offline
  verifiability, non-repudiation, privacy against the operator, trust
  boundaries between providers, and availability through fallbacks---in
  a new, less structured setting.
...
\begin{description}
  \item[Denial-of-service protection] How do we prevent attackers from 
    ...
    communications?
    Rate limiting, proof-of-work~\autocite{DworkNaor1992pow}, and physical
    design (tamper-resistant radios) are partial solutions.

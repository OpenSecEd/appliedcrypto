...
\ltnote{%
  The idea is to use a more open form of pQBL~\parencite{pQBL}.
...
These videos are also strictly confidential, since the privacy of many
people is at stake (whoever is captured in the videos).
...
We can use some form of secure element to store the key, \eg a \ac{TPM} or
an HSM (Hardware Security Module) or a smart card (think SIM card).
...
We will use \ac{AES} to do encryption.
...
\begin{exercise}
  ...
  of AES (128 bits\footnote{The block size should not be confused with the
  key size.}).
\end{exercise}

We can see in \cref{fig:EncModes} that it is very important to pick a good
mode of operation~\autocite{blockmodes} in this context (videos).
We will use \ac{GCM}~\autocite{McGrewViega2004gcm,SP80038D} to get
authenticated encryption.
This mode has a built-in MAC\@.
We will need a random IV (initialization vector).
We can use a key-derivation function (such as HKDF~\autocite{HKDF}) to
derive the encryption key from \(\KCR\):
\begin{align*}
  \KENC &= \KDF[\Enc, \KCR]. \\
  ...
  c_i &= m_i \oplus \Enc[\KENC, \IV \concat i]
  \text{ in line with \ac{GCM} (a counter mode),} \\
  t &\text{ is the built-in MAC tag, computed over all of }
  c_1, \ldots, c_n.
\end{align*}
The decryption fails if the built-in MAC is invalid---that is, if any \(c_i\)
has been tampered with.
Tampered with, in this context, includes bit modifications and that someone
has dropped a few bits or even an entire block.
...
\begin{figure}
  \begin{sidecaption}[Tux encrypted using different modes.]{%
    ...
    When using \ac{ECB} mode (\cref{fig:EncModes:TuxECB}), we can still
    distinguish Tux.
...
One caution applies to both variants: the IV must never repeat under the
same key.
In counter-based modes such as \ac{GCM}, encrypting twice with the same
key--IV pair is catastrophic for both confidentiality and integrity.
The camera must therefore ratchet to a fresh key and IV for every new
...
\begin{figure}
  \begin{sidecaption}[Transfer data from \(C\) to \(R\).]{%
    ...
    It first encrypts the video \(m\) as it is captured, so that it stores
    only \(c\).
    It identifies as \(C\) so that \(R\) knows where the video material
...
\ltnote{%
  The object of learning is integrity granularity: a single tag over the
  whole video can only answer yes or no, whereas chained per-chunk tags
  localize tampering and let us salvage the intact parts.
  What varies is how the authentication is composed: one built-in tag over
  the entire stream versus explicit chained MACs over chunks.
  What remains invariant is the counter-mode encryption, the keys derived
  from \(\KCR\), and the upload flow.
  The critical aspect is the chunking of the authentication, not the switch
  away from \ac{GCM} itself---the body therefore points out that chunked
  \ac{GCM} messages would achieve the same granularity.
}

Now we will use \ac{CTR} (instead of \ac{GCM}) and use HMAC separately
to ensure integrity.
We will use \ac{CTR} mode as this mode is lightweight and highly
parallelizable (same benefit as in \ac{GCM}).
...
We will use HMAC-SHA256~\autocite{HMAC,shs} for this.
...
\begin{figure}
  \begin{sidecaption}[Transfer data from \(C\) to \(R\) using HMAC]
    {%
      ...
      so that it stores only \(c = c_0, \ldots, c_{N-1}, t_0, c_N, \ldots\).
      It identifies as \(C\) so that \(R\) knows where the video material
      comes from, but more importantly so that \(R\) knows which key to use
      (\(\KCR\)).
      Once the encrypted video material \(c\) is uploaded, the reviewers can 
  ...
  \begin{msc}
    ...
    {Transfer data from \(C\) to \(R\) using HMAC}
      ...
      \mess{I'm camera \(C\).}{cam}{rev}
      \nextlevel[2]
      \mess{\(c = c_0, \ldots, c_{N-1}, t_0, c_N, \ldots, c_{2\cdot
      N-1}, t_1, \ldots\)}{cam}{rev}
      \nextlevel
...
Or reorder them.
Or modify them.
We would detect exactly where the problem is and we would see that other
parts are intact---this we couldn't do with a single \ac{GCM} tag over the
whole video.
Note that this gain comes from chunking the authentication, not from
abandoning \ac{GCM}: we could equally well have split the video into
per-minute \ac{GCM} messages and chained them together.
...
This change improves the situation over \ac{GCM}\@.
...
\ltnote{%
  The object of learning is that symmetric authentication is
  verifier-forgeable: whoever can verify a MAC can also compute one.
  What varies is the authentication primitive---MAC versus digital
  signature; what remains invariant is the chunked, chained message
  structure and the symmetric encryption.
  The critical aspect is the separation of the verification capability from
  the signing capability, and how this redistributes trust: the courts no
  longer need to trust the reviewers not to fabricate footage.
  A secondary critical aspect surfaces in the consequences: the signature
  scheme reintroduces a randomness requirement that the symmetric design
  had eliminated, preparing the contrast developed in
  \cref{AsymmetricEncryption}.
}
...
We will use ECDSA (Elliptic Curve Digital Signature
Algorithm)~\autocite{FIPS186-5} with a curve that is secure for the key
size we choose.

We should look at the recommendations from NIST~\autocite{SP80057} or
similar standards, but the curves \texttt{P-256} or \texttt{P-384} are
...
\begin{description}
  \item[Secure key storage] \(\SKC\) must be stored in a secure element in the 
    camera (\eg \ac{TPM} or HSM); any compromise must be reported and a new
    key issued.
  ...
  \item[Randomness] The signatures require randomness.
    ...
    However, one could use constructions such as
    RFC~6979~\autocite{RFC6979} to deterministically calculate the nonce
    in a secure way, and thus avoid the need for generating randomness.
...
\ltnote{%
  This section repeats the symmetric-to-asymmetric move of
  \cref{DigitalSignatures}, but for confidentiality instead of
  authenticity---a generalization pattern: the same kind of change, applied
  to the other security property.
  What varies is the encryption primitive; what remains invariant is the
  signature chain and the chunked structure.
  The two paired exercises on nonces (why a deterministic nonce is fatal
  for encryption but fine for signatures) form a deliberate contrast on
  the role of randomness: same word, \enquote{nonce}, two different
  critical requirements (unpredictability versus mere uniqueness).
  The hybrid-encryption subsection then resolves the efficiency problem
  while keeping the new trust structure invariant.
}
...
\begin{solution}
  We want the encryption system to be (at least) \emph{semantically
  secure}~\autocite{GoldwasserMicali1984}.
...
The algorithm and padding can be chosen by looking at the PKCS standards,
\eg PKCS\ \#1~\autocite{RFC8017} to use RSA~\autocite{RSA1978} with
OAEP~\autocite{BellareRogaway1994oaep}.
...
encryption}~\autocite{CramerShoup2003hybrid}:
...
\begin{description}
  \item[Compromise containment] Session keys are ephemeral---compromise of a
    single session key does not expose earlier material.
    If a camera is lost or physically attacked, previous footage remains
    secure (no persistent symmetric key stored).
    Only the current session can be decrypted.
    Note that this is \emph{not} forward
    secrecy~\autocite{PerfectForwardSecrecy,AuthKeyExchange}: every session
    key is encrypted under \(\PKR\), so compromise of the long-term private
    key~\(\SKR\) still exposes all past material.
  \item[Flexible access control] Video chunks can, in principle, be encrypted 
    ...
    of the data for all.
  \item[Key management] Each camera must be provisioned with the reviewers'
    (or authority's) public key \(\PKR\) during manufacturing or issuing.
    ...
    We can use a \ac{PKI} to manage the public/private
    key pairs.
...
\begin{description}
  \item[Timing attacks] If the time to compute a signature depends on the
    secret key bits, an attacker measuring operation times can reconstruct
    the key~\autocite{Kocher1996timing}.
  \item[Power analysis] The power consumption of a chip varies depending on
    which operations it performs.
    By measuring power traces during cryptographic operations, attackers
    can extract secret keys~\autocite{KocherJaffeJun1999dpa}.
  \item[Electromagnetic emanations] Similar to power analysis, but
    measuring electromagnetic radiation from the
    chip~\autocite{QuisquaterSamyde2001ema}.
\end{description}
...
\begin{description}
  \item[Constant-time implementations] Ensure all operations take the
    same time regardless of key bits.
    This is standard practice in modern cryptographic libraries.
  \item[Power filtering and noise injection] Add capacitors and noise
    generators to mask power consumption patterns.
  \item[Shielding] Metal enclosures reduce electromagnetic emanations.
  \item[Randomized execution] Add random delays and dummy operations to
    decorrelate power traces from actual computations.
\end{description}

Using a certified secure element (such as a \ac{TPM} or smartcard meeting
Common Criteria EAL4+ or FIPS 140-3 Level 3) provides hardware-level
...
\ltnote{%
  The object of learning is that deletion is a protocol step that needs
  cryptographic evidence, just like transmission.
  What varies is the camera's deletion trigger: believing an upload
  happened versus verifying a signed receipt for it.
  What remains invariant is the chunk format and signature chain from the
  previous sections.
  The critical aspect is non-repudiation of \emph{receipt}---the dual of
  the non-repudiation of \emph{origin} that the camera's signatures
  already provide.
  A secondary critical aspect is key separation: the server signs receipts
  with a dedicated key pair rather than reusing its encryption keys, so
  students see one-key-one-purpose modelled rather than violated.
}
...
For this, the reviewers' server gets its own signing key pair
\((\SKSrv, \VKSrv)\), separate from the encryption key pair
\((\PKR, \SKR)\)---one key pair, one purpose (key separation).
Each camera is provisioned with \(\VKSrv\) so that it can verify the
server's receipts:
...
\begin{enumerate}
  \item The camera (\(C\)) collects and stores encrypted video chunks
    ...
    non-volatile memory.
  \item Upon reaching a docking station or establishing a reliable
    connection, the camera initiates an upload session with the reviewers'
    server (\(R\)).
  \item The camera transmits each chunk to the reviewers' server.
  \item For each chunk (or batch of chunks), the reviewers' server computes
    a cryptographic hash digest (\(h\)) of the received data and returns a
    server-generated digital signature (\(\tau\)) on the tuple (camera
    identifier, chunk indices, received hashes, timestamp, etc.):
    \[
      \tau_i = \Sign[\SKSrv, (\text{camera ID}, i, h(c_i), t)]
    \]
  \item The camera verifies \(\tau_i\) using the server's verification
    key~\(\VKSrv\).
  \item Only after the camera has received and verified all the relevant
    signatures (\(\tau\)) for the uploaded data, does it mark those chunks
    as safely received and deletes them from local storage.
\end{enumerate}

This ensures that until receipt has been cryptographically acknowledged, the
footage remains on the camera, making accidental (or deliberate) removal
much harder.
...
\begin{figure}
  \begin{sidecaption}[Robust upload protocol with explicit receipt]
    {%
      ...
      Each uploaded chunk is only deleted after the reviewers' server returns
      a digital signature acknowledging receipt and integrity.
    }[fig:TransferCRReceipt]
  \flushright
  \begin{msc}
    [/msc/action width=0.4\linewidth]
    {Robust upload from \(C\) to \(R\) with explicit receipt}
      \declinst{cam}{Camera \(C\)}{\(\SKC, \VKSrv\)}
      \declinst{rev}{Reviewers \(R\)}{\(\SKR, \SKSrv\)}
      \mscaction{For each chunk \(c_i, \sigma_{i-1}, \sigma_i\) in local
      storage}{cam}
      \nextlevel[2]
      \mess{Send chunk \((c_i, \sigma_{i-1}, \sigma_i)\)}{cam}{rev}
      \mscaction{Compute \(h = H(c_i, \sigma_{i-1}, \sigma_i)\)}{rev}
      \mscaction{Generate receipt signature \(\tau_i = \Sign[\SKSrv,
      (\text{camera ID}, i, h, t)]\)}{rev}
      \nextlevel[2]
      \mess{Receipt signature \(\tau_i\)}{rev}{cam}
      \mscaction{Verify signature \(\Ver[\VKSrv, (\text{camera ID}, i, h, t),
      \tau_i]\)}{cam}
...
For even greater assurance, cameras could be configured to maintain a
rolling buffer, retaining a fixed period (\eg one week) of footage locally,
regardless of receipt acknowledgements, unless specifically commanded
otherwise over a secure management interface.
This further reduces the risk that footage is lost due to mistakes, local
storage failures at reviewers, or other exceptional events.

Additionally, upload could be performed to multiple geographically separated
servers (georedundancy), requiring acknowledgements from each before
permitting deletion.

\subsection{Detection of missing or tampered uploads}

Because each chunk (and its associated signatures and hashes) forms a
cryptographically linked chain (as in \cref{DigitalSignatures}), reviewers
(or external auditors) can easily detect gaps, missing data, or reordering
in the server-side archives---and these will be detectable by the camera's
deletion logic as well.
...
\begin{description}
  \item[Strong non-repudiation] The server's signed receipt forms a
    durable record confirming which data was received, when, and by whom.
  \item[Reduced opportunity for tampering or data loss] A corrupt police
    officer \(P\) cannot cause premature deletion of footage by faking
    uploads or disrupting connectivity, nor can a rogue server operator
    delete evidence without leaving a cryptographically accountable trail.
  \item[Storage management tradeoff] Cameras need sufficient non-volatile
    storage to buffer footage awaiting upload and receipt; storage limits
    might delay full deletion of old material.
  \item[Server private key sensitivity] Loss or compromise of the server's
    signing key~\(\SKSrv\) would affect trust in receipts; this key must be
    tightly managed and, ideally, backed with hardware security controls.
  \item[Operational complexity] More sophisticated protocols increase
    system complexity and introduce the need for robust handling of
    communication failures, partial uploads, or out-of-order receipt
    handling.
\end{description}

Overall, this enhancement makes the system much more robust to both
accidental and malicious data loss during the upload process.
The protocol also leaves an accountable, auditable chain of custody for all
video material from capture to deletion.
...
To make the audit log tamper-evident, we use a hash
chain~\autocite{SchneierKelsey1999logs}.
...
external timestamp authority~\autocite{HaberStornetta1991,RFC3161} or
...
These algorithms are secure against classical computers, but would be
broken by a sufficiently powerful quantum computer running Shor's
algorithm~\autocite{Shor1997}.
...
\begin{description}
  \item[Hybrid encryption] Use both a classical algorithm (RSA/ECDH) and a
    post-quantum algorithm (such as ML-KEM, formerly
    Kyber~\autocite{FIPS203}) for key encapsulation.
  ...
  \item[Larger symmetric keys] AES-256 provides 128-bit security against
    quantum attacks (due to Grover's algorithm~\autocite{Grover1996}),
  ...
  \item[Post-quantum signatures] For new deployments, use post-quantum
    signature algorithms such as ML-DSA (formerly
    Dilithium)~\autocite{FIPS204} or SLH-DSA (formerly
    SPHINCS+)~\autocite{FIPS205}.
...
\begin{description}
  \item[Redactable signatures] Use a signature scheme that allows
    designated portions to be removed while the signature on remaining
    portions stays valid~\autocite{JMSW2002redactable}.
    This is an active research area with schemes based on chameleon
    hashes~\autocite{KrawczykRabin2000chameleon}.
...
\begin{figure}
  ...
  \begin{msc}
    ...
    {Video redaction}
      ...
      \nextlevel
      \mscaction{Verify \(\sigma_R\), optionally request original for
      audit}{court}
      \nextlevel
  \end{msc}
  \end{sidecaption}
...
A complete body camera system combines:
\begin{enumerate}
  \item Hybrid (envelope) encryption of all video under the reviewers'
    public key~\(\PKR\), with a fresh session key per recording.
  \item Per-camera signing keys~\(\SKC\) with chained signatures over the
    encrypted chunks, so that anyone can verify authenticity
    using~\(\VKC\).
  \item A receipt-based upload protocol: the camera deletes footage only
    after verifying the server's signed receipt.
  \item Tamper-evident audit logging with externally timestamped
    checkpoints for the chain of custody.
  \item Hybrid classical/post-quantum key encapsulation for long-term
    confidentiality.
  \item A dual-signature redaction workflow that preserves the original
...
\ltnote{%
  ...
  time in this chapter---composing confidentiality and integrity,
  symmetric versus asymmetric trust structures, key separation and
  management, evidence for protocol steps (receipts, audit logs), and
  long-term security---in a new, less structured setting.
...
\begin{description}
  \item[Deepfake detection and prevention] As synthetic media becomes more 
    ...
    watermarks (such as C2PA~\autocite{C2PAspec}), and AI-based tampering
  ...
  \item[Automated \ac{GDPR} compliance] Tools for automatic face detection,
    consent management, and retention policy enforcement could reduce the
    burden on reviewers while ensuring legal compliance.
...
Christoffer Eklund,
Christoforos Seas,
Deniel Kenadid Saskin,
Eduardo Santos Rodriguez,
Edwin Ahlstrand,
Elias Nygren,
...
Max Andersson,
Melvin Sandberg,
Miguel Pinheiro Coelho,
Miguel Valgañon Soria,
Nils Eric Jordan,
Oscar George Judd,
Oskar Vesterlund,
Paula Sopena Coello,
Pontus Hashmi Hector,
Praveen Kannappan Manikandan,
Sever Mihai Nastase,
Shreyas Sawai,
Simon Michel Jean-Noël Hamelin,
Stella Carlsson,
Thi Huyen Trang Nguyen,
Timothy Lindquist Marongiu,
Tim Rundström,
Tomás Silva da Fonseca,
